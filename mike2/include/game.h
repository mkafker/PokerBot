...
namespace Poker {
    ...
    class GameState {
        ...
        public:
            ...
            unique_ptr<Player> currentPlayer;   // At the beginning of each round, points to the player we're waiting on to make a move
            map<unique_ptr<Player>, int> playerBetAmounts;
            // The below are derivable from the above, do I need them?
            int pot;
            int minimumBet;

            shared_ptr<Deck> getDeck() const;
            void shuffleDeck();
            ...
            vector<shared_ptr<Player>> getPlayingPlayersInBettingOrder(vector<shared_ptr<Player>> in = vector<shared_ptr<Player>>());
    };
    class Game {
        public:
            ...
            // These functions modify currentGS
        ...
        void Game::resetCards(random_device& rd) {
