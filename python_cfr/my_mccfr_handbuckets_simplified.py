...
_MAX_BET = 2 # Includes the antee of $1
_STREET = 1
_HAND_BINS = 4
_NB_SIMULATION = 2000
_N_ITERATIONS = 10
_load_previous_imap = False
#random.seed(3)
 
#P1;1;3;aa/c1r
#{player_id};{hand_strength};{street};{actions}
...
def main():
    ...
    if _load_previous_imap:
        try:
            with open(f'trained_model_street-{_STREET}.pickle', 'rb') as f:
                i_map = pickle.load(f)
    ...
    for i in range(_N_ITERATIONS):
        ...
        expected_game_value += mccfr(i_map, start_from, _STREET)
        ...
        if (i+1) % 10 == 0:
            ...
            try:
                ...
                with open(f'trained_model_street-{_STREET}.json', 'w') as f:
                    json.dump(temp_i_map, f, default=custom_serializer, indent=4)
                with open(f'trained_model_street-{_STREET}.pickle', "wb") as f:
                    pickle.dump(i_map, f)
            except Exception as e:
                print(e)
...
def mccfr(i_map, start_from, street, infoset_key=";;;", deck = Deck(13, 4), 
    ...
    infoset_key : A key to represent the game tree path we have taken
        Ex: "P1;1;3;aa/cc/c1r", "PlayerID; Current Hand Strength; Street; History"
    ...
    if is_chance_node(infoset_key):
        return chance_util(i_map, start_from, street, infoset_key, deck, 
    ...
    for i, action in enumerate(valid_actions(infoset_key, _MAX_BET)):
        next_infoset_key = swap_players(infoset_key, p1_hs, p2_hs) + action
        
        if player_id == "P1":
            action_utils[i] = -1 * mccfr(i_map, start_from, street, next_infoset_key, deck, 
                                    p1_cards, p2_cards, co_cards,
                                    p1_hs, p2_hs,
    ...
    # if infoset_key == "P1;0;1;aa/c1r":
    #     print(pr_2, strategy, action_utils, util, regrets, info_set.regret_sum, estimate_hand_strength(p1_cards, co_cards, deck))

    return util

def swap_players(infoset_key, p1_hs, p2_hs):
...
def chance_util(i_map, start_from, street, infoset_key, deck, 
    p1_cards, p2_cards, co_cards, 
    p1_hs, p2_hs,
    pr_1, pr_2, pr_c):
    ...
    if len(action_history[0]) == 0: # Initializing
        expected_value = 0
        ...
        # for deal in [deck.draw_random_cards(2)]: # Bad MC-CFR, too much MC
            ...
            s_cards, new_deck = new_deck.draw_random_cards(2)
            co_cards, final_deck = new_deck.draw_random_cards(cards_to_draw)
            ...
            if start_from == "P1":
                ...
                p1_hs = str(p1_hand_strength)
                p2_hand_strength = clamp_hand_strength(estimate_hand_strength(s_cards, co_cards, final_deck))
                p2_hs = str(p2_hand_strength)
                infoset_key = f'P1;{p1_hand_strength};{street};aa/'
                expected_value += mccfr(i_map, start_from, street, infoset_key, final_deck, 
                ...
                p2_hs = str(p2_hand_strength)
                infoset_key = f'P2;{p2_hand_strength};{street};aa/'
                
                p1_hand_strength = clamp_hand_strength(estimate_hand_strength(s_cards, co_cards, final_deck))
                p1_hs = str(p1_hand_strength)
                p1_infoset_key = f"P1;{p1_hand_strength};{street};aa/"
                ...
                for i, action in enumerate(valid_actions(p1_infoset_key, _MAX_BET)):
                    ...
                    expected_value += strategy[i] * mccfr(i_map, start_from, street, next_infoset_key, final_deck, 
