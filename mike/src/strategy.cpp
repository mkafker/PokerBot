...
namespace Poker {
    ...
    PlayerMove Strategy::simpleDressMove(const Move move, const float raiseMultiplier, const int minBet, const int bankroll) {
      auto clamp = [](int a, int b, int c) -> int { if(a<b) a=b; if(a>c) a=c; return a;};
      int bet_amount = 0;
      Move newMove = move;
      if( move == Move::MOVE_FOLD) bet_amount = 0;
      else if( move == Move::MOVE_CALL) bet_amount = minBet;
      else if( move == Move::MOVE_RAISE) bet_amount = minBet * raiseMultiplier;
      else if( move == Move::MOVE_ALLIN) bet_amount = bankroll;
      bet_amount = clamp(bet_amount, 0, bankroll);
      if( bet_amount == bankroll) newMove = Move::MOVE_ALLIN;
      return PlayerMove(newMove, bet_amount);
    }
    PlayerMove RandomAI::makeMove(const shared_ptr<Table>& info, Player* p) {
        ...
        switch( dist4(rng) ) {
            case 1:
                myMove = Move::MOVE_FOLD;
                break;
            case 2:
                myMove = Move::MOVE_CALL;
                break;
            case 3:
                myMove = Move::MOVE_RAISE;
                break;
            case 4:
                myMove = Move::MOVE_ALLIN;
                break;
        }
        return simpleDressMove( myMove, 2.0, info->minimumBet, p->bankroll );
    }

    PlayerMove Strategy::makeMove(const shared_ptr<Table>& info, Player* p) {
      // Default behavior: always call
      return simpleDressMove( Move::MOVE_CALL, 2.0, info->minimumBet, p->bankroll );
    }
